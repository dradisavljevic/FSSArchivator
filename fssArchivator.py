    ...
    with open(cfg.export_file_name, 'a') as csvFile:
        ...
        for j in range(0,len(season_links)):
            if season_links[j].text.strip().replace('"','')=='2005-2006':
                break
            football_clubs, identifier = scrape_match(season_links[j], driver, 1, football_clubs, identifier, writer)
    csvFile.close()

    driver.close()

def scrape_match(link, driver, league_level, football_clubs, identifier, writer):
    league_season = link.text.strip().replace('"','')
    driver.get(cfg.root_link+link['href'])

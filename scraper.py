...
def main():
    ...
    for k in range(0, len(league_systems)):
        ...
        write_to_csv(filename=export_files[k], webpage_content=webpage_content, driver=driver)
...
def write_to_csv(filename, webpage_content, driver):
        ...
        for j in range(0,len(season_links)):
            ...
            if season_links[j].text.strip().replace('"', '') != cfg.ONGOING_SEASON:
                ...
                football_clubs, identifier, scraped_leagues, links_to_scrape =\
                 scrape_league_season(driver, football_clubs, identifier, writer, scraped_leagues, links_to_scrape, link=season_links[j], league_level=level)
    csvFile.close()

    return football_clubs, scraped_leagues, links_to_scrape

# Crawls through league seasons until it reaches the cutoff season
def scrape_league_season(driver, football_clubs, identifier, writer, scraped_leagues, links_to_scrape, link, league_level):
...
def get_next_league(webpage_content, driver, links_to_scrape, scrape_link, league_level):
    ...
    for j in range(0,len(league_level_tabs)):
        next_league = league_level_tabs[j].find('a')
        if j != 0:
            # The first tab is already clicked on by default
            webdriver.ActionChains(driver).move_to_element(league_level_tab_buttons[j]).click(league_level_tab_buttons[j]).perform()
        webpage_content = get_webpage_content(driver)
